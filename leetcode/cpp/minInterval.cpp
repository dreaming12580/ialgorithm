...
struct Event {
    // 事件的类型， 0表示左端点，1表示询问， 2表示右端点
    int type;
    // 事件的位置
    int pos;
    // 事件的额外参数
    // 如果类型是0左端点事件， 那么para 记录右端点的位置
    // 如果类型是1询问事件， 那么para记录它是第几个询问，以便于返回答案数组
    int para;

    Event(int _type, int _pos, int _para) :
        type(_type), pos(_pos), para(_para) {}
